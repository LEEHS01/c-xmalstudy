...
using iWaterDataCollector.Kafka.Impl.Handler;
using iWaterDataCollector.Kafka.Internal;
using iWaterDataCollector.Kafka.Internal.Extensions;
...
namespace iWaterDataCollector.Kafka
{
    ...
    internal class Consumer<TKey, TValue> : IConsumer<TKey, TValue>, IClient
    {
        ...
        internal Consumer(ConsumerBuilder<TKey, TValue> builder)
        {
            ...
            var config = iWaterDataCollector.Kafka.System.Config.ExtractCancellationDelayMaxMs(baseConfig.config, out this.cancellationDelayMaxMs);
        ...
        public ConsumeResult<TKey, TValue> Consume(int millisecondsTimeout)
        {
            ...
            try
            {
                ...
                try
                {
                    unsafe
                    {
                        key = keyDeserializer.Deserialize(msg.key == IntPtr.Zero ? ReadOnlySpan<byte>.Empty : new ReadOnlySpan<byte>(msg.key.ToPointer(), (int)msg.key_len), msg.key == IntPtr.Zero, new SerializationContext(MessageComponentType.Key, topic, headers));
                    }
                }
                catch (Exception ex)
                {
