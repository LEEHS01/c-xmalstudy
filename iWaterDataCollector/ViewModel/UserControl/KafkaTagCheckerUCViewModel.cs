...
{
    public class KafkaTagCheckerUCViewModel : ViewModelBase, IDataErrorInfo
    {
        ...
        public string FilterString
        {
            ...
            set
            {
                if (_filterString != value)
                {
                    ...
                    FilteringAction(string.IsNullOrEmpty(_filterString));
        ...
        public bool IsFilter
        {
            ...
            set
            {
                ...
                {
                    _isFilter = value;
                    RaisePropertyChanged(nameof(IsFilter));
                }
            }
        ...
        /// 생성자
        /// </summary>
        public KafkaTagCheckerUCViewModel()
        {
        ...
        private void ReflashList()
        {
            ...
            //Kafka Tag List Load
            var lkafka = FileHandler.ReadKafakCollection(AppData.Instance.KafkaPath, out string date);
            if (lkafka.Count > 0)
            {
                try
                {
                    foreach (var item in lkafka)
                    {
                        //쉼표(,)로 Parsing
                        var aInfo = item.Split(',');
                        //Pulse Tag 여부확인
                        _ = bool.TryParse(aInfo[2], out bool isPulse);
                        //Pulse Tag 유지시간 확인
                        _ = int.TryParse(aInfo[4], out int duration);
                        var tag = new TagModel(aInfo[0], aInfo[1], true, isPulse);
                        //Pulse Tag인 경우 tagModel 내의 Pulse정보를 추가
                        if (isPulse)
                        {
                            tag.Pulse = new PulseTagModel(aInfo[3], aInfo[5], duration);
                        }
                        DisplayList.Add(tag);
                    }
                    ...
                    if (DisplayList.Count > 0)
                    {
                        //DataGrid Row가 있는 경우 가장 첫번째 Tag 선택하도록 설정
                        SelectedTag = DisplayList[0];
                    }
                    //상단 Kafka Tag 설정 날짜 표출
                    CreateDate = date;
                    AppData.Instance.MsgIRDC.Info(AppData.AppLog, _myName, "Kafka Tag List Load 완료");
                }
                catch(Exception ex)
                {
                    AppData.Instance.MsgIRDC.Error(AppData.ErrorLog, _myName, "Kafka Tag Parsing 실패", ex);
                }
            }
            else
            {
                AppData.Instance.MsgIRDC.Warn(AppData.AppLog, _myName, "Kafka Tag File Load 실패");
            }
        }
        #endregion
        #region 명령등록
        ...
        private void Command_Handler(object obj)
        {
            ...
            switch (command)
            {
                ...
                case COMMAND_ROW_ADD:
                    try
                    {
                        DisplayList.Add(new TagModel(string.Empty, string.Empty, true, false));
                    }
                    catch (Exception ex)
                    {
        ...
        private void Save()
        {
            ...
            var ltag = DisplayList.Select(t => string.Join(",", t.Name, t.Description, t.IsSelected, t.Pulse.OnValue, t.Pulse.DuringTime, t.Pulse.OffValue)).ToArray();
            //기존파일 백업
            FileHandler.MoveRecoveryCSVFile(AppData.Instance.KafkaPath);
            //설정된 경로에 csv파일로 저장
            if (FileHandler.WriteCSVFile(AppData.Instance.KafkaPath, ltag))
            {
                MessageBox.Show("Kafak Tag 리스트가 저장되었습니다.", "파일저장", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            else
            {
                MessageBox.Show("kafka Tag 리스트 저장을 실패하였습니다.", "파일저장", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        }
        /// <summary>
        /// 이중 예외처리
